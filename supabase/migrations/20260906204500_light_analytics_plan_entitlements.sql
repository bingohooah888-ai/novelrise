-- LIGHT ANALYTICS entitlement boundary.
-- Free receives only an all-works aggregate. Standard receives per-work funnel and
-- Standard plan-extra effects. Premium receives Standard data plus Premium-only slot
-- effects. Unknown or missing plan state fails closed to Free-level aggregation.
-- Production application requires separate OWNER approval.

begin;

select pg_advisory_xact_lock(hashtext('novelrise:20260906204500'));
...
begin
  ...
  -- profiles.plan is synchronized by NOVELIGHT's entitlement flow. Any missing or
  -- unexpected value intentionally collapses to Free instead of opening detail data.
...
-- Keep the legacy v1 endpoint from becoming a Free-plan bypass. It projects the
-- already-entitlement-filtered v2 result into the legacy shape.
