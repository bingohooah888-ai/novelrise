-- Hotfix: valid official thumbnail uploads were reaching Storage but failing
-- database registration because the SECURITY DEFINER RPC used an over-escaped
-- PostgreSQL regular expression for storage paths.
--
-- Keep the table constraint, authorization boundary, audit behavior, and all
-- business validation unchanged. Use [.] for the literal extension separator
-- so the expression is independent of backslash/string-literal semantics.

begin;

select pg_advisory_xact_lock(hashtext('novelight:20260911123000:thumbnail-asset-path'));

do $$
begin
  if to_regprocedure(
    'public.novelight_admin_register_thumbnail_layer_asset(uuid,text,text,text,text,text,integer,text)'
  ) is null then
    raise exception 'novelight_admin_register_thumbnail_layer_asset is missing';
  end if;
end
$$;
...
-- Migration-local postcondition: the canonical path shape that the API creates
-- must be accepted by the corrected PostgreSQL expression.
do $$
begin
  if 'official/00000000-0000-0000-0000-000000000000.png'
       !~ '^official/[0-9a-f-]{36}[.](webp|png|jpg|jpeg)$' then
    raise exception 'corrected official thumbnail storage path expression rejected canonical path';
  end if;
end
$$;
