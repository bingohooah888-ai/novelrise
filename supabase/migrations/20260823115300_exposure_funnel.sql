-- NOVELIGHT exposure-to-reading funnel attribution.
-- Attribute reader actions only to a real recent impression from the same viewer.
-- Raw viewer keys stay server-side; authors receive aggregate funnel metrics only.

begin;

select pg_advisory_xact_lock(hashtext('novelrise:20260823115300'));

create table public.novel_exposure_conversions (
  id uuid primary key default gen_random_uuid(),
  exposure_id uuid not null references public.novel_exposure_events(id) on delete cascade,
  viewer_key_snapshot text not null,
  novel_id_snapshot text not null,
  author_id_snapshot uuid not null,
  event_type text not null check (event_type in ('detail_open', 'episode_read_10s')),
  episode_id_snapshot text,
  converted_at timestamptz not null default now(),
  constraint novel_exposure_conversion_once unique (exposure_id, event_type),
  constraint novel_exposure_episode_requirement check (
    (event_type = 'detail_open' and episode_id_snapshot is null)
    or (event_type = 'episode_read_10s' and episode_id_snapshot is not null)
  )
);

create index novel_exposure_conversion_author_idx
  on public.novel_exposure_conversions (author_id_snapshot, converted_at desc);

create index novel_exposure_conversion_novel_idx
  on public.novel_exposure_conversions (novel_id_snapshot, converted_at desc);

revoke all on table public.novel_exposure_conversions from public, anon, authenticated;
...
begin
  ...
  -- Never count an author's own browsing as discovery success.
  ...
  -- Attribution window: the latest real impression for this viewer/work in the
  -- previous 24 hours. Direct URL traffic has no matching exposure and returns false.
  select e.id
  into v_exposure_id
