...
-- Rollback removes only telemetry introduced by this migration so the prior
-- two-value event constraint can be restored. It does not touch preregistration rows.
delete from public.beta_author_preregistration_events
 where event_type in ('form_start', 'register_click');
...
comment on table public.beta_author_preregistration_events is
  'Private beta-author LP KPI events. Public clients may only append page_view/cta_click events through the same-origin server API.';
