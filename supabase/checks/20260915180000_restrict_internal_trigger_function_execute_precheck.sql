...
begin
  foreach v_name in array array[
    ...
    if v_security_definer is distinct from true or v_return_type <> 'trigger' then
      raise exception 'public.%() has unexpected trigger security semantics', v_name;
    end if;
  ...
  if v_oid is not null then
    if v_security_definer is distinct from true or v_return_type <> 'event_trigger' then
      raise exception 'public.rls_auto_enable() has unexpected event-trigger security semantics';
    end if;
    select count(*) into v_binding_count from pg_event_trigger where evtfoid = v_oid;
    if v_binding_count < 1 then
      raise exception 'public.rls_auto_enable() has no active event-trigger binding';
    end if;
    if not has_function_privilege('service_role', v_oid, 'EXECUTE') then
