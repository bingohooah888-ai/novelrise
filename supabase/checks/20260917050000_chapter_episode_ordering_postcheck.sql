...
begin
  ...
  if not has_function_privilege('anon', 'public.novelight_novel_outline(bigint)', 'EXECUTE')
     or not has_function_privilege('authenticated', 'public.novelight_novel_outline(bigint)', 'EXECUTE') then
    raise exception 'Public outline RPC must be available to readers and owners';
  end if;
