...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (event.type === 'customer.subscription.deleted') {
      ...
      if (findError) {
        throw new Error(`Profile lookup failed: ${findError.message}`);
      }

      if (!profiles || profiles.length === 0) {
        throw new Error(`Profile not found for customerId: ${customerId}`);
      }
      ...
      if (updateError) {
        throw new Error(`Plan reset failed: ${updateError.message}`);
      }
