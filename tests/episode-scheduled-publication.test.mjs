...
test('scheduled publication keeps episodes private drafts until the due worker publishes them', () => {
...
test('scheduled publication reuses publication safety rules and auto-clears stale schedules', () => {
  ...
  assert.match(migration, /Episode title must contain 1 to 150 characters/u);
  assert.match(migration, /Episode content must contain 1 to 100000 characters/u);
...
test('draft editor exposes one-shot schedule controls only after the schema supports them', () => {
...
test('draft manager remains compatible before migration and shows schedule state afterwards', () => {
