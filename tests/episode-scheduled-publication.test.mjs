...
test(
  'scheduled publication keeps episodes private drafts until database publication',
  () => {
    assert.match(migration, /add column scheduled_publish_at timestamptz/u);
    assert.match(
      migration,
      /scheduled_publish_at is null or status = 'draft'/u
    );
...
test(
  'scheduled publication preserves owner and internal execution boundaries',
  () => {
    assert.match(
      migration,
...
test(
  'draft editor exposes scheduling only after the database column exists',
  () => {
