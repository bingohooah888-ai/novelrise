...
test('ledger distinguishes mutation result from postcheck result', () => {
  ...
  assert.match(bridge, /failure_phase='workflow'/);
  assert.match(
    bridge,
    /MUTATION_RESULT: \$\{\{ needs\.repair\.outputs\.mutation_result \}\}/
  );
  assert.match(
    bridge,
    /POSTCHECK_RESULT: \$\{\{ needs\.repair\.outputs\.postcheck_result \}\}/
  );
  assert.match(
    bridge,
    /FAILURE_PHASE: \$\{\{ needs\.repair\.outputs\.failure_phase \}\}/
  );
  ...
  assert.match(
    bridge,
    /result:\$result,mutation_result:\$mutation_result,postcheck_result:\$postcheck_result,failure_phase:\$failure_phase/
  );
