...
async function login(page, account, redirect) {
  ...
  await page.waitForURL((url) => url.pathname.endsWith(`/${redirect.split('?')[0]}`));
  const visitorToken = await page.evaluate(() => localStorage.getItem('novelight_visitor_token'));
  expect(visitorToken).toBeTruthy();
  return visitorToken;
}

test('authenticated beta-critical product flow works in production', async ({ browser, baseURL }) => {
  const fixture = loadFixture();
  ...
  try {
    const authorVisitorToken = await login(authorPage, fixture.author, 'post.html');
    ...
    await authorPage
      .locator('#description')
      .fill('β公開前の認証済み本番スモーク専用作品です。テスト終了時に自動削除されます。');
    ...
    await authorPage
      .locator('#content')
      .fill('これはNOVELIGHTの本番認証スモーク用本文です。読書画面と10秒読書記録を検証します。');
    ...
    await expect(authorPage.locator('.episode-title').first()).toHaveText(episodeTitle);
    const episodeHref = await authorPage.locator('.episode-title').first().getAttribute('href');
    ...
    await expect(readerPage.locator('#seedMessage')).toContainText('すでに贈っています');

    await readerPage.goto('/scout-record.html');
    await expect(readerPage.getByRole('heading', { name: 'SCOUT RECORD' })).toBeVisible();
    ...
    await expect(readerPage.locator('#card .content')).toContainText('本番認証スモーク用本文');
    await readerPage.waitForTimeout(10_500);

    await authorPage.goto('/analytics.html');
    await expect(authorPage.getByRole('heading', { name: 'LIGHT ANALYTICS' })).toBeVisible();
