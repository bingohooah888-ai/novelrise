...
const [mypage, retirement, originalMigration] = await Promise.all([
  ...
  readFile(
    'supabase/migrations/20260906204000_retire_author_gallery_feature.sql',
    'utf8'
  ),
  readFile(
...
test('author room no longer loads or exposes the free-form gallery', async () => {
  ...
  await assert.rejects(readFile('novelight-author-gallery.js', 'utf8'), {
    code: 'ENOENT'
  });
  await assert.rejects(readFile('novelight-author-gallery.css', 'utf8'), {
    code: 'ENOENT'
  });
});
...
test('gallery retirement is forward-only and preserves existing Storage data', () => {
  ...
  assert.match(
    retirement,
    /revoke all on function public\.novelight_author_gallery_can_upload_v1/u
  );
  assert.match(
    retirement,
    /drop policy if exists "Author gallery insert own"/u
  );
  assert.match(
    retirement,
    /drop policy if exists "Author gallery delete own"/u
  );
