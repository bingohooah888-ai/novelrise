...
const css = fs.readFileSync(path.join(root, 'novelight-login-parchment.css'), 'utf8');

test('login loads the parchment visual layer without replacing auth wiring', () => {
  ...
  assert.ok(login.includes('supabaseClient.auth.signInWithPassword({email,password})'));
});

test('login parchment styling keeps the approved palette and fixes header sizing', () => {
