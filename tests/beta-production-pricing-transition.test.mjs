...
test('Production subscription transition stops recurring Standard 980 without a new charge and migrates Premium without proration', async () => {
  ...
  assert.match(
    transition,
    /stripe\.subscriptions\.cancel\(subscription\.id,[\s\S]*invoice_now: false,[\s\S]*prorate: false/u
  );
  assert.match(
    transition,
    /stripe\.subscriptions\.update\(subscription\.id,[\s\S]*price: premiumPriceId/u
  );
...
test('beta Standard migration has a fail-closed rollback artifact', async () => {
  ...
  assert.match(
    rollback,
    /beta_standard_rollback_requires_profile_reconciliation/u
  );
  assert.match(
    rollback,
    /drop function if exists public\.novelight_activate_beta_standard/u
  );
