...
test('beta admin API treats lifecycle milestones as forward-only', () => {
  ...
  assert.match(adminApi, /function isBackwardLifecycleTransition\(current, nextStatus\)/);
  assert.match(adminApi, /const currentRank = lifecycleRankForRecord\(current\)/);
  assert.match(adminApi, /nextRank < currentRank/);
  assert.match(adminApi, /if \(isBackwardLifecycleTransition\(current, status\)\)/);
...
test('email verification and invite lifecycle states are system-owned', () => {
  assert.match(adminApi, /SYSTEM_OWNED_STATUSES = new Set\(\['verified', 'invited'\]\)/);
  assert.match(
    adminApi,
...
test('beta admin UI disables lifecycle choices below the achieved milestone floor', () => {
  assert.match(
    adminHtml,
  ...
  assert.match(adminHtml, /systemOwned\|\|\(Number\.isInteger\(optionRank\)/);
