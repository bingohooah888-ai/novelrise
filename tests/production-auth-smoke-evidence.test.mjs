...
test('request-only workflow can never be PASS evidence', () => {
  const result = evaluate({
    run: run({ name: 'NOVELIGHT Production Auth Smoke Request' })
  });
  assert.equal(result.pass, false);
  assert.match(result.reason, /unexpected workflow/);
});

test('workflow-dispatch forwarding run can never be PASS evidence', () => {
  const result = evaluate({ run: run({ event: 'workflow_dispatch' }) });
  assert.equal(result.pass, false);
  assert.match(result.reason, /unexpected workflow event/);
});
