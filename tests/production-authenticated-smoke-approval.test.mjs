...
test('production authenticated smoke uses one-time chat approval instead of manual environment approval', async () => {
  const workflow = await text('.github/workflows/production-authenticated-smoke.yml');
  ...
  assert.match(workflow, /Production Authenticated Smoke approval request expired/);
  assert.match(workflow, /main changed after the user approved this Production Authenticated Smoke/);
  assert.match(workflow, /Re-check approved main immediately before Production write/);
  assert.doesNotMatch(workflow, /environment: production-approval/);
});

test('production authenticated smoke stays SHA-bound and cleans ephemeral production data', async () => {
  const workflow = await text('.github/workflows/production-authenticated-smoke.yml');

  assert.match(workflow, /ref: \$\{\{ steps\.approval\.outputs\.main_sha \}\}/);
  assert.match(workflow, /environment: Production/);
  assert.match(workflow, /Create ephemeral production smoke users/);
  assert.match(workflow, /Run authenticated production smoke with runner Chrome/);
