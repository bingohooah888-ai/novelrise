...
test('auth smoke approval handler uses one-time chat approval', async () => {
  ...
  assert.ok(
    workflow.startsWith('name: NOVELIGHT Production Auth Smoke Approval Handler')
  );
...
test(
  'request creation is a separately named workflow, not smoke proof',
  async () => {
    const handler = await text(handlerPath);
    const request = await text(requestPath);

    assert.ok(
      request.startsWith('name: NOVELIGHT Production Auth Smoke Request')
    );
    ...
    assert.ok(
      !request.includes('Verify authenticated beta-critical production flows')
    );
...
test(
  'only the verification job is authoritative Auth Smoke PASS evidence',
  async () => {
    const workflow = await text(handlerPath);

    assert.ok(
      workflow.includes('name: Verify authenticated beta-critical production flows')
    );
    assert.ok(workflow.includes("github.event_name == 'issue_comment'"));
    assert.ok(
      workflow.includes(
        "startsWith(github.event.comment.body, 'NOVELIGHT_PRODUCTION_AUTH_SMOKE_APPROVE ')"
      )
    );
    assert.ok(
      workflow.includes('if: success() && steps.approval.outputs.request_id !=')
    );
    assert.ok(workflow.includes('NOVELIGHT_PRODUCTION_AUTH_SMOKE_CONSUMED'));
    assert.ok(
      workflow.includes('if: failure() && steps.approval.outputs.request_id !=')
    );
    assert.ok(workflow.includes('NOVELIGHT_PRODUCTION_AUTH_SMOKE_FAILED'));
  }
);
...
test(
  'request workflow stays SHA-bound before creating approval issue',
  async () => {
    const request = await text(requestPath);
