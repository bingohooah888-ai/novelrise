...
test(
  'Production billing health supports approved manual audit and gated scheduled audit',
  async () => {
...
test(
  'legacy webhook cleanup remains approval-gated and proves the canonical no-charge path',
  async () => {
    const workflow = await text(
...
test(
  'Production control proof includes the real checkout API without a live payment',
  async () => {
...
test(
  'incident runbook makes evidence-first diagnosis and automation escalation explicit',
  async () => {
    const runbook = await text(
      'docs/PRODUCTION-BILLING-INCIDENT-RUNBOOK.md'
    );
