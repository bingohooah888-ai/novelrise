...
function paidDependencies(portalError) {
  const supabase = {
    auth: {
      async getUser() {
        return { data: { user: { id: 'user-paid', email: 'paid@example.com' } }, error: null };
      }
    },
    from(table) {
      ...
      return {
        ...
        async limit() {
          return {
            data: [{ plan: 'premium', payment_status: 'active', stripe_customer_id: 'cus_missing', subscription_status: 'active' }],
            error: null
          };
        }
      };
  ...
  return {
    ...
    stripe: {
      billingPortal: {
        sessions: {
          async create() { throw portalError; }
        }
      }
    },
    env: {
...
test('paid Premium profile pointing at a missing Stripe customer returns an actionable 409', async (t) => {
  ...
  const stripeError = Object.assign(new Error('No such customer'), {
    name: 'Error', type: 'StripeInvalidRequestError', code: 'resource_missing', param: 'customer'
  });
  ...
  assert.deepEqual(state.body, { error: 'Billing account needs repair', code: 'billing_state_conflict' });
});

test('unrelated portal failures remain generic server errors', async (t) => {
  t.mock.method(console, 'error', () => {});
  const handler = createCheckoutHandler(paidDependencies(new Error('Portal unavailable')));
  const { res, state } = responseState();
  await handler({ method: 'POST', headers: { authorization: 'Bearer token-123' }, body: { plan: 'premium' } }, res);
