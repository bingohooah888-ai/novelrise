...
test(
  'Chapter 39 stores seven official layer IDs at fixed 1086x1448 geometry',
  () => {
...
test(
  'official material tables are RLS protected and browser writes stay closed',
  () => {
    assert.match(
      migration,
...
test(
  'legacy complete thumbnails remain compatible during rolling deployment',
  () => {
    assert.match(
      migration,
...
test(
  'cover mask is internal and enforced before a composition can be stored',
  () => {
...
test(
  'cached render is derived WebP and source layer IDs remain canonical',
  () => {
...
test(
  'reader cards prefer one cached image and can rebuild missing cache from official layers',
  () => {
...
test(
  'rollback refuses destructive reversal after layered compositions are in use',
  () => {
    assert.match(
      rollback,
