...
test(
  'Free LIGHT ANALYTICS is aggregate-only in the client and fails closed on plan lookup errors',
  () => {
    assert.match(analytics, /let days=30,plan='free'/u);
    assert.match(analytics, /function normalizePlan\(value\)/u);
    assert.match(
      analytics,
      /if\(p\.error\)\{console\.error\(p\.error\);plan='free'\}/u
    );
    assert.match(
      analytics,
      /if\(plan==='free'\)renderFreeScope\(\);else render\(rows\)/u
    );
...
test(
  'server-side analytics returns only aggregate data for Free and per-work data for paid tiers',
  () => {
    assert.match(migration, /v_plan text := 'free'/u);
    assert.match(
      migration,
      /from public\.profiles p[\s\S]*where p\.id = v_uid/u
    );
    assert.match(migration, /v_plan := coalesce\(v_plan, 'free'\)/u);
    assert.match(migration, /where v_plan in \('standard', 'premium'\)/u);
    assert.match(
      migration,
      /null::text,[\s\S]*null::text,[\s\S]*coalesce\(sum\(a\.impressions\), 0\)::bigint/u
    );
    assert.match(migration, /having v_plan = 'free'/u);
    assert.match(
      migration,
      /case when v_plan = 'premium' then a\.premium_impressions else 0::bigint end/u
    );
    assert.match(
      migration,
      /from public\.novelight_author_exposure_funnel_v2\(p_days\) f/u
    );
  }
);

test(
  'Standard and Premium client views keep additional exposure effects separated',
  () => {
...
test(
  'pricing terminology remains aligned without changing prices or checkout behavior',
  () => {
