...
test(
  'badge artwork zoom copies the active detail artwork and supports keyboard access',
  async () => {
    ...
    assert.ok(
      script.includes("document.getElementById('badgeDialogArtworkImage')")
    );
...
test('badge artwork zoom is a large centered modal surface', async () => {
  ...
  assert.match(
    styles,
    /width: min\(640px, 80vw, calc\(100dvh - 112px\)\);/u
  );
