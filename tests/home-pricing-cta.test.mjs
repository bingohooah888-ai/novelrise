...
test('home pricing cards expose all three beta plan CTAs', () => {
  assert.match(home, /<a class="plan-cta" href="signup\.html">無料で始める<\/a>/u);
  assert.match(home, /<a class="plan-cta" href="pricing\.html">Standardを無料で利用<\/a>/u);
  assert.match(home, /<a class="plan-cta" href="pricing\.html">Premiumを始める<\/a>/u);
...
test('home pricing CTAs stay bottom-aligned and Standard remains primary', () => {
  ...
  assert.match(home, /\.plan\.recommended \.plan-cta\{background:var\(--primary\);color:#fff\}/u);
});
