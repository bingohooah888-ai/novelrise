...
test(
  'Staging sync request bridge is owner-only and bound to the control issue',
  () => {
    ...
    assert.match(requestWorkflow, /NOVELIGHT_STAGING_MIGRATION_SYNC /);
  }
);

test(
  'Staging sync request requires exact SHA, one migration, and typed confirmation',
  () => {
    ...
    assert.match(requestWorkflow, /main changed before the request was claimed/);
  }
);

test('Staging sync request is one-time and records a durable ledger', () => {
  assert.match(requestWorkflow, /NOVELIGHT_STAGING_MIGRATION_SYNC_CLAIMED /);
  assert.match(requestWorkflow, /NOVELIGHT_STAGING_MIGRATION_SYNC_CONSUMED /);
...
test(
  'Staging request bridge delegates to the existing fail-closed sync workflow',
  () => {
    assert.match(
      requestWorkflow,
...
test(
  'Staging request bridge never receives database or Production credentials',
  () => {
