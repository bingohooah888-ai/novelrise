...
const migrationUrl=new URL('../supabase/migrations/20260917123000_author_work_export.sql',import.meta.url);
const rollbackUrl=new URL('../supabase/rollback/20260917123000_author_work_export_rollback.sql',import.meta.url);
const precheckUrl=new URL('../supabase/checks/20260917123000_author_work_export_precheck.sql',import.meta.url);
const postcheckUrl=new URL('../supabase/checks/20260917123000_author_work_export_postcheck.sql',import.meta.url);
const uiUrl=new URL('../my-novels.html',import.meta.url);
const exportClientUrl=new URL('../novelight-author-work-export.js',import.meta.url);
...
test('work export authorization is bound to auth.uid and owner-only',async()=>{
  ...
  assert.match(migration,/novelight_authorize_work_export\(\s*p_novel_id bigint,\s*p_format text/i);
  ...
  assert.match(migration,/grant execute on function public\.novelight_authorize_work_export\(bigint, text\) to authenticated/i);
  assert.match(migration,/revoke all on function public\.novelight_authorize_work_export\(bigint, text\) from anon/i);
});
...
test('work export audit is private and rate limits are concurrency-safe',async()=>{
  ...
  assert.match(migration,/revoke all on table public\.author_work_export_audit from authenticated/i);
  ...
  assert.match(postcheck,/anonymous role can execute author export authorization/i);
});
...
test('author backup UI exports owned manuscript data as TXT without evaluation metrics',async()=>{
  ...
  assert.doesNotMatch(client,/\bpv\b|favorite_count|light_seed|scout_xp|work_rank|final_rank/i);
...
test('migration safety artifacts match the authenticated RPC signature',async()=>{
  ...
  for(const artifact of [migration,rollback,precheck,postcheck]){
    assert.match(artifact,/novelight_authorize_work_export\(bigint(?:, |,)text\)/i);
    assert.doesNotMatch(artifact,/novelight_authorize_work_export\(uuid,bigint,text\)/i);
  }
  assert.match(rollback,/drop table if exists public\.author_work_export_audit/i);
