...
test('thumbnail remains required and is always inserted into payload', () => {
  ...
  assert.match(postHtml, /if\(!thumbnailAsset\)\{status\.textContent='作品に合う画像を1枚選んでください。';return\}/);
  assert.match(postHtml, /thumbnail_asset_id:thumbnailAsset/);
  assert.doesNotMatch(postHtml, /if\(thumbnailAsset\)payload\.thumbnail_asset_id=thumbnailAsset/);
});
