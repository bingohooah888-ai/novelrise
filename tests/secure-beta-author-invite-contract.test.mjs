...
test(
  'secure invite ledger is private and never persists raw bearer tokens',
  () => {
...
test(
  'Before User Created requires invite proof during preopen and protects reserved emails after launch',
  () => {
...
test(
  'invite consumption explicitly binds Founding identity and scrubs transport metadata',
  () => {
    const consume =
      migration.match(
...
test(
  'signup keeps invite token in the URL fragment and only submits it after server validation',
  () => {
...
test(
  'ADMIN outbound send is authenticated, idempotent and gated away from preregistration',
  () => {
...
test(
  'campaign transition fails closed until invite storage and Resend secret are ready',
  () => {
...
test(
  'manual ADMIN cannot forge email verification or invite-sent milestones',
  () => {
    assert.match(
      adminApi,
...
test(
  'pre/postchecks and rollback preserve fail-closed identity protection',
  () => {
    assert.match(
      precheck,
      /Secure invite migration requires PRE_REGISTRATION/
    );
