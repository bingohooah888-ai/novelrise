...
test('secure invite ledger is private and never persists raw bearer tokens', () => {
  ...
  assert.match(migration, /alter table public\.beta_author_invites enable row level security/);
  assert.match(
    migration,
...
test('signup keeps invite token in the URL fragment and only submits it after server validation', () => {
  ...
  assert.match(signup, /if\(inviteValidated\)signupMetadata\.novelight_invite_token=inviteToken/);
