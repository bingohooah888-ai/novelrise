...
const migration = await readFile(
  new URL('supabase/migrations/20260913112358_trusted_public_impression_receipts.sql', root),
  'utf8'
);
const discovery = await readFile(new URL('novelight-discovery-list.js', root), 'utf8');
...
test('anonymous author-facing impressions still require short-lived server-issued receipts', () => {
  ...
  assert.match(migration, /viewer_id is null[\s\S]*viewer_key like 'visitor:%'/u);
  ...
  assert.match(migration, /grant execute on function public\.record_trusted_allocation_receipts_v2\(uuid\[\], text\) to anon, authenticated/u);
  assert.match(migration, /revoke all on function public\.record_novel_impressions\(text, text\[\], text\) from public, anon, authenticated/u);
  assert.match(migration, /revoke all on function public\.record_novel_impressions_v2\(text, text\[\], text\) from public, anon, authenticated/u);
});

test('public shelf receipt issuer validates the claimed surface against its authoritative feed', () => {
  assert.match(migration, /novelight_issue_visible_allocation_receipts_v2_impl/u);
  ...
  assert.match(migration, /One or more works are not eligible for the claimed rendered surface/u);
...
test('new privileged helpers stay in the private schema behind invoker-only public wrappers', () => {
  ...
  assert.doesNotMatch(migration, /public\.novelight_trusted_discovery_feed_v2[\s\S]{0,500}security definer/u);
});

test('dedicated discovery pages consume author-facing receipts only after cards are appended', () => {
  const newBlock = discovery.match(/async function loadNew\(\) \{([\s\S]*?)\n  \}/u)?.[1] || '';
  const seedBlock = discovery.match(/async function loadSeed\(\) \{([\s\S]*?)\n  \}/u)?.[1] || '';
  assert.ok(newBlock.indexOf('appendRows(rows)') < newBlock.indexOf("recordVisible('search_new'"));
  assert.ok(seedBlock.indexOf('appendRows(page)') < seedBlock.indexOf("recordVisible('search_seed'"));
...
test('home records only viewport-visible rows after DOM rendering', () => {
  assert.match(home, /grid\.innerHTML=visible\.length\?visible\.map\(card\)\.join\(''\)/u);
  assert.match(home, /if\(visible\.length\)void issueAndRecord\('home_new',visible\)/u);
  assert.match(home, /if\(seeded\.length\)void issueAndRecord\('home_seed',seeded\)/u);
