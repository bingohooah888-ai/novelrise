...
test('homepage header uses the official NOVELIGHT logo asset', () => {
  ...
  assert.ok(statSync(logoPath).size > 1_000, 'official header logo asset must not be empty');
});

test('homepage logo stays within the desktop and mobile header', () => {
  assert.match(home, /\.logo img\{[^}]*height:44px[^}]*max-width:42vw[^}]*\}/);
  assert.match(home, /@media\(max-width:860px\)\{[^}]*[\s\S]*?\.logo img\{height:34px\}/);
});
