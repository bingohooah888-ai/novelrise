...
test('signup beta Standard copy', () => {
  assert.match(signup, /β期間中はStandardを無料・カード登録不要で利用できます/u);
  assert.match(signup, /会員登録後、料金プランから有効化してください/u);
});

test('signup confirmation guides authors', () => {
  assert.match(signup, /確認後は「創作室」から投稿を始められます/u);
  assert.match(signup, /Standard無料利用は料金プランから有効化できます/u);
  assert.match(signup, /カード登録不要/u);
});

test('signup keeps auth and billing behavior unchanged', () => {
  const redirect = "emailRedirectTo:window.location.origin+'/index.html'";
  assert.ok(signup.includes(redirect));
