...
test('signup confirmation tells authors where to continue after email verification', () => {
  assert.match(signup, /確認後は「創作室」から投稿を始められます/u);
  assert.match(signup, /Standard無料利用は料金プランから有効化できます/u);
  assert.match(signup, /カード登録不要/u);
});

test('signup onboarding copy does not change the existing auth or billing contract', () => {
  assert.match(
    signup,
