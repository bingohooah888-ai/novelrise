...
test(
  'account settings verifies the logged-in user before rendering private email',
  () => {
...
test(
  'account settings changes email only through the official logged-in Auth API',
  () => {
...
test(
  'account settings separates validation, request and confirmation-pending states',
  () => {
...
test(
  'Author Studio and mypage expose matching interaction and account settings navigation',
  () => {
    assert.match(
      shell,
      /\['interaction-settings\.html', '⚙', '交流設定'\]/u
    );
    assert.match(
      shell,
      /\['account-settings\.html', '⌘', 'アカウント設定'\]/u
    );
...
test(
  'public reader and author surfaces do not gain a public account-email source',
  () => {
    for (const source of [
      authorPublic,
      searchPublic,
...
test(
  'preregistration email remains historical admin data rather than current login email',
  () => {
...
test(
  'Founding and beta qualification remain anchored to auth_user_id after registration',
  () => {
    assert.match(
      qualificationMigration,
...
test(
  'AUTHOR_PREOPEN continues to gate initial signup on preregistration email only',
  () => {
