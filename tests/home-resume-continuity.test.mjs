...
test('home resume reads only valid NOVELIGHT progress and orders it newest first', () => {
...
test('home resume continues the current episode until 85 percent, then advances', () => {
...
test('home page schedules the resume enhancement after DOMContentLoaded only', () => {
  assert.match(authReaderSource, /novelight-home-resume\.js/);
  assert.match(authReaderSource, /DOMContentLoaded/);
  assert.match(authReaderSource, /pathname === '\/' \|\| pathname\.endsWith\('\/index\.html'\)/);
});
