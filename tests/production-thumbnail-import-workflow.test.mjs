...
test(
  'thumbnail Production import passes issue comment body into validation shell',
  () => {
    assert.match(
      workflow,
...
test('thumbnail Production import keeps fail-closed approval scope checks', () => {
  ...
  assert.match(
    workflow,
    /Safety stop: main changed after thumbnail import approval\./
  );
  assert.match(
    workflow,
    /Safety stop: thumbnail import approval was already consumed\./
  );
  assert.match(
    workflow,
    /Safety stop: transfer branch SHA does not match approval\./
  );
});
