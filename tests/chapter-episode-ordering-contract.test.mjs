...
test(
  'chapter storage is owner-private and chapter membership cannot cross novels',
  () => {
...
test(
  'structure reorder is atomic and preserves the legacy unique episode numbering contract',
  () => {
...
test(
  'chapter metadata remains navigation-only and cannot alter evaluation systems',
  () => {
...
test(
  'public outline exposes published episodes only while owner can inspect drafts',
  () => {
    ...
    assert.match(
      outline,
      /grant execute on function public\.novelight_novel_outline\(bigint\) to anon, authenticated/i
    );
  }
);

test(
  'author structure manager supports chapter CRUD, assignment and whole-work reorder',
  () => {
...
test(
  'novel page renders chapter headings from the safe outline RPC and links owners to manager',
  () => {
...
test(
  'direct episode number editing is retired in favor of atomic structure management',
  () => {
...
test(
  'migration ships explicit precheck, postcheck and guarded rollback artifacts',
  () => {
