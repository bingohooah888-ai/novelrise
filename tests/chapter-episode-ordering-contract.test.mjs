...
const migration = await readFile(
  'supabase/migrations/20260917050000_chapter_episode_ordering.sql',
  'utf8'
);
const precheck = await readFile(
  'supabase/checks/20260917050000_chapter_episode_ordering_precheck.sql',
  'utf8'
);
const postcheck = await readFile(
  'supabase/checks/20260917050000_chapter_episode_ordering_postcheck.sql',
  'utf8'
);
const rollback = await readFile(
  'supabase/rollback/20260917050000_chapter_episode_ordering_rollback.sql',
  'utf8'
);
...
test('chapter storage is owner-private and chapter membership cannot cross novels', () => {
  ...
  assert.match(migration, /alter table public\.novel_chapters enable row level security/i);
  assert.match(
    migration,
    /foreign key \(chapter_id, novel_id\)[\s\S]*references public\.novel_chapters\(id, novel_id\)/i
  );
  assert.match(migration, /revoke all on table public\.novel_chapters from public, anon, authenticated/i);
  assert.match(migration, /create policy novel_chapters_select_owner/i);
  assert.doesNotMatch(migration, /grant select on table public\.novel_chapters to anon/i);
});

test('structure reorder is atomic and preserves the legacy unique episode numbering contract', () => {
  const reorderStart = migration.indexOf('create function public.novelight_reorder_novel_structure');
  assert.ok(reorderStart >= 0);
  const reorder = migration.slice(reorderStart, migration.indexOf('create function public.novelight_novel_outline'));

  assert.match(reorder, /security definer/i);
  assert.match(reorder, /set search_path = ''/i);
  assert.match(reorder, /for update/i);
  assert.match(reorder, /Episode ordering must contain every episode exactly once/i);
  assert.match(reorder, /Chapter ordering must contain every chapter exactly once/i);
  assert.match(reorder, /set episode_number = -e\.episode_number/i);
  assert.match(reorder, /with ordinality/i);
  assert.match(reorder, /set episode_number = desired\.new_number,[\s\S]*chapter_id = desired\.chapter_id/i);
...
test('public outline exposes published episodes only while owner can inspect drafts', () => {
  const outlineStart = migration.indexOf('create function public.novelight_novel_outline');
  assert.ok(outlineStart >= 0);
  const outline = migration.slice(outlineStart);

  assert.match(outline, /v_is_owner := v_user_id is not null and v_user_id = v_owner_id/i);
  assert.match(outline, /if not v_is_owner and v_novel_status <> 'published' then[\s\S]*return;/i);
  assert.match(outline, /v_is_owner[\s\S]*e\.status = 'published'/i);
  assert.match(outline, /grant execute on function public\.novelight_novel_outline\(bigint\) to anon, authenticated/i);
});
...
test('author structure manager supports chapter CRUD, assignment and whole-work reorder', () => {
  ...
  assert.ok(structurePage.includes("client.rpc('novelight_create_novel_chapter'"));
  assert.ok(structurePage.includes("client.rpc('novelight_rename_novel_chapter'"));
  assert.ok(structurePage.includes("client.rpc('novelight_delete_novel_chapter'"));
  assert.ok(structurePage.includes("client.rpc('novelight_reorder_novel_structure'"));
...
test('direct episode number editing is retired in favor of atomic structure management', () => {
  ...
  assert.ok(!episodeEdit.includes('episode_number:value.episodeNumber,title:value.title'));
...
test('migration ships explicit precheck, postcheck and guarded rollback artifacts', () => {
  ...
  assert.match(rollback, /drop function if exists public\.novelight_novel_outline/i);
