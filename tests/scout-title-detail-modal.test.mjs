...
test('Scout title detail dialog is explicitly centered and enlarged', () => {
  ...
  assert.match(
    css,
    /\.scout-modal\{[\s\S]*?position:fixed;[\s\S]*?top:50%;[\s\S]*?left:50%;/u
  );
  assert.match(css, /transform:translate\(-50%,-50%\)/u);
  assert.match(
    css,
    /#badgeDialog\.scout-modal\{width:min\(760px,calc\(100vw - 40px\)\)\}/u
  );
...
test('Scout title detail dialog stays centered and viewport-safe on mobile', () => {
  assert.match(
    css,
    /\.scout-modal\{width:calc\(100vw - 20px\);max-height:calc\(100dvh - 20px\)\}/u
  );
  assert.match(
    css,
    /#badgeDialog\.scout-modal\{width:calc\(100vw - 20px\)\}/u
  );
