...
test(
  'authenticated smoke follows the integrated SCOUT RECORD seed-history surface',
  () => {
    assert.match(scoutRecord, /<h2 id="activityTitle">最近のSCOUT活動<\/h2>/);
    assert.match(
      scoutRecord,
      /<h2 id="seedHistoryTitle">LIGHT SEED送信履歴<\/h2>/
    );
    assert.match(scoutRecord, /id="seedHistoryList"/);
    assert.doesNotMatch(
      scoutRecord,
      /href="light-seed-history\.html">SEED履歴 →<\/a>/
    );
