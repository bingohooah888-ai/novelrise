...
test('assistant auto-resume contract avoids needless continuation prompts without weakening safety', async () => {
  ...
  assert.match(gate, /MASTER-first違反として扱ってユーザーへ新しい「はい」「続けて」を要求してはならない/);
  ...
  assert.match(gate, /外部request \/ claim \/ mutationがまだ一度も開始されていない/);
  assert.match(gate, /final-head SHA \/ challengeへ固定されたHigh-Risk PR承認/);
