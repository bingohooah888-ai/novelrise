...
const registrySource = readFileSync(new URL('../docs/DOCUMENT-SOURCE-OF-TRUTH.md', import.meta.url), 'utf8');
...
test('registry defines one current MASTER and no first-PR delete candidates', () => {
  ...
  assert.deepEqual(indexed.currentByRole.get('master'), ['docs/NOVELIGHT-MASTER.md']);
...
test('historical release records are archived while dated counsel handoff stays current', () => {
  assert.equal(entry('docs/BETA-RELEASE-EVIDENCE-2026-08-23.md').status, 'ARCHIVED');
  assert.equal(entry('docs/BETA-RELEASE-EVIDENCE-2026-08-26.md').status, 'ARCHIVED');
  assert.equal(entry('docs/BETA-RELEASE-DECISION-2026-08-28.md').status, 'ARCHIVED');
  assert.equal(entry('docs/LEGAL-COUNSEL-HANDOFF-2026-08-28.md').status, 'CURRENT');
});
...
test('archived or superseded files cannot be selected as Current State', () => {
  assert.throws(
    () => validateSelectedCurrentDocuments(registry, ['docs/BETA-RELEASE-EVIDENCE-2026-08-23.md']),
    /cannot be used as Current State/
  );
  assert.throws(
    () => validateSelectedCurrentDocuments(registry, ['docs/exposure-allocation-beta.md']),
    /cannot be used as Current State/
  );
});
...
test('duplicate current singleton role fails closed', () => {
  ...
  assert.throws(() => indexRegistry(broken), /Singleton role master|MASTER Source of Truth/);
});

test('superseded document without a current same-role successor fails closed', () => {
  const broken = clone(registry);
  const successor = broken.documents.find((item) => item.path === 'docs/exposure-allocation-beta-v2.md');
...
test('registry coverage detects unregistered formal docs and missing registered docs', () => {
  ...
  let errors = verifyRegistryCoverage({ registry, formalPaths: [...formal, 'docs/UNREGISTERED.md'] });
  assert.ok(errors.some((message) => message.includes('UNREGISTERED')));
  errors = verifyRegistryCoverage({ registry, formalPaths: formal.filter((path) => path !== 'docs/NOVELIGHT-ADMIN.md') });
...
test('current references reject archived dependencies unless explicitly historical', () => {
  const miniature = {
    ...
    documents: [
      { path: 'docs/NOVELIGHT-MASTER.md', status: 'CURRENT', role: 'master', singletonRole: true, historicalReferences: [] },
      { path: 'docs/HISTORY.md', status: 'ARCHIVED', role: 'history', singletonRole: false, historicalReferences: [] }
    ]
  };
  const files = new Map([
...
test('missing current document references fail structural verification', () => {
  ...
  const master = miniature.documents.find((item) => item.path === miniature.masterPath);
...
test('fast manifest records exact main, registry, MASTER and selected guide blobs', () => {
  const blobs = new Map(registry.documents.map((item, index) => [item.path, index.toString(16).padStart(40, '0')]));
  ...
  assert.ok(manifest.selectedDocuments.some((item) => item.path === 'docs/STAGING-RUNBOOK.md'));
});

test('document reference extraction is bounded to formal markdown paths', () => {
  assert.deepEqual(
    extractDocumentReferences('See `docs/ONE.md`, [two](docs/TWO.md), AGENTS.md and src/app.js.'),
...
test('verify mode detects repo/document registry drift without mutating files', { concurrency: false }, () => {
  ...
  const mini = {
    ...
    registryPath: 'docs/DOCUMENT-SOURCE-OF-TRUTH.md',
    masterPath: 'docs/NOVELIGHT-MASTER.md',
    startupRequiredCurrent: ['docs/NOVELIGHT-MASTER.md', 'docs/DOCUMENT-SOURCE-OF-TRUTH.md'],
    deleteCandidates: [],
    holdCandidates: [],
    documents: [
      { path: 'docs/DOCUMENT-SOURCE-OF-TRUTH.md', status: 'CURRENT', role: 'document-source-of-truth-registry', singletonRole: true, historicalReferences: [] },
      { path: 'docs/NOVELIGHT-MASTER.md', status: 'CURRENT', role: 'master', singletonRole: true, historicalReferences: [] }
    ]
  };
...
test('runtime wrapper reuses already-fetched origin/main and persists compact freshness basis', { concurrency: false }, () => {
  const root = mkdtempSync(join(tmpdir(), 'novelight-runtime-doc-freshness-'));
  sh(root, ['git', 'init', '-q']);
  sh(root, ['git', 'config', 'user.email', 'fixture@example.com']);
  sh(root, ['git', 'config', 'user.name', 'fixture']);
  sh(root, ['git', 'remote', 'add', 'origin', 'https://example.invalid/novelight.git']);
  ...
  writeFileSync(
    ...
    readFileSync(new URL('../scripts/document-freshness-lib.mjs', import.meta.url), 'utf8'),
    'utf8'
  );
  writeFileSync(
    join(scriptRoot, 'runtime-gate-entry.mjs'),
    readFileSync(new URL('../scripts/runtime-gate-entry.mjs', import.meta.url), 'utf8'),
    'utf8'
  );
  writeFileSync(
  ...
  const output = execFileSync(process.execPath, ['scripts/runtime-gate-entry.mjs', '--phase=start'], {
    cwd: root,
    encoding: 'utf8'
  });
  assert.equal(output, '');
  const state = JSON.parse(readFileSync(join(root, '.git', 'novelight-runtime-gate.json'), 'utf8'));
  assert.equal(state.version, 12);
  assert.equal(state.documentFreshness.mainSha, head);
  assert.equal(state.documentFreshness.master.path, 'docs/NOVELIGHT-MASTER.md');
