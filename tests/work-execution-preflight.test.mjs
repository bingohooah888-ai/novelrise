...
test('work execution preflight requires visible total-time reporting before tools', async () => {
...
test('work execution preflight requires short external waits to continue automatically', async () => {
  const source = await readPreflight();

  assert.match(source, /短時間外部待機・自動継続ゲート/);
  assert.match(source, /概ね10分以内/);
  assert.match(source, /同じターンで結果確認・必要なログ診断・安全に自動実行できる次工程まで続行/);
