...
function has(text, token) {
  assert.equal(text.includes(token), true, `expected SQL to include ${token}`);
}
...
test('replay rebuild excludes self-comments before applying the daily cap', () => {
  ...
  has(migration, 'when n.id is not null then e.user_id is distinct from n.user_id');
...
test('postcheck rejects self-comment EXP and replay drift', () => {
  ...
  has(postcheck, 'Comment XP ledger does not match self-comment-safe replay rules');
});
...
test('migration replay exercises hardening rollback and restores current rules', () => {
  ...
  const applyCount = replay.split(
    'supabase/migrations/20260910143000_chapter38_exclude_self_comment_scout_exp.sql'
  ).length - 1;
  assert.ok(applyCount >= 2, 'self-comment hardening must be reapplied after rollback');
});
