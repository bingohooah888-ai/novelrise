...
test('novel detail warning gate does not load full novel data or telemetry before confirmation', () => {
  assert.match(
    novelHtml,
    /select\('id,user_id,title,genre,ai_usage,status,content_rating,content_warnings,created_at'\)/
  );
  assert.match(novelHtml, /if\(needsWarningGate\(\)&&!warningAccepted\(\)\)\{showWarningGate\(\);return\}/);
  assert.match(novelHtml, /async function loadFullNovelAndRender\(\)/);
  assert.match(novelHtml, /rememberWarningAccepted\(\);await loadFullNovelAndRender\(\)/);
...
test('author room self-heals missing profiles and avoids DOM global metrics', () => {
  ...
  assert.match(mypageHtml, /const st=document\.getElementById\('analyticsStatus'\),metrics=\{/);
...
test('novel editing requires an official thumbnail', () => {
  assert.match(novelEditHtml, /作品に合う画像<span class="required">必須<\/span>/);
  assert.match(novelEditHtml, /if\(!thumbnailAsset\)\{status\.textContent='作品に合う画像を1枚選んでください。';return\}/);
...
test('database boundary enforces beta runtime rules', () => {
  ...
  assert.match(migration, /create or replace function public\.novelight_ensure_my_profile\(\)/);
  assert.match(migration, /char_length\(v_content\) > 100000/);
  assert.match(migration, /p_episode_number is null or p_episode_number < 1/);
  assert.match(migration, /char_length\(v_title\) < 1 or char_length\(v_title\) > 150/);
});
