...
test(
  'creator pages use only the shared frameless page-heading treatment',
  async () => {
    for (const [path, nativeClass] of pages) {
      ...
      assert.match(
        source,
        new RegExp(
          'class="[^"]*' + nativeClass + '[^"]*novelight-page-heading[^"]*"'
        ),
        path
      );
...
test(
  'page-heading stylesheet cannot restyle page typography or content surfaces',
  async () => {
    ...
    assert.doesNotMatch(
      styles,
      /font-size|font-family|\.panel|\.card|input|select|textarea/
    );
  }
);
