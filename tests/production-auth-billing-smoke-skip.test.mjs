...
test('production auth smoke can skip staging-only billing setup', async () => {
  ...
  assert.ok(source.includes("const completeBilling = process.env.E2E_COMPLETE_BILLING === 'true';"));
  ...
  assert.ok(!source.includes('const stagingSupabase = resolveStagingSupabaseOverride();'));
});
