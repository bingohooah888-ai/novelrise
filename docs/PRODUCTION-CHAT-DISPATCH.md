...
This route remains baseline-only and retired in place on the legacy ledger. If the baseline is already SATISFIED/APPLIED, it must not be rerun. It is deliberately not activated on issue #657.
...
- trigger: a new owner-authored approval record on active Production Approval Ledger issue #657
- approved ref: exact current `main` SHA
...
This legacy record format is retained only for audit/contract continuity. The baseline route is not active on issue #657.
...
- the event is a new comment on issue #657, not a pull request;
...
The workflow re-checks `main`, then records its own `..._CLAIMED` marker immediately before entering the Production execution job. The Production job independently re-reads issue #657 and requires exactly one matching claim bound to the same workflow run ID, SHA, challenge, operation, and exact migration scope.
...
For normal migrations, `.github/workflows/production-migration-approved-dispatch.yml` performs the Production deploy directly from its owner-only issue #657 `issue_comment` trigger after the exact migration approval has been validated and claimed.

Before mutation it must:

- re-check current `main` against the approved SHA before claim;
- re-read issue #657 at the Production boundary and require exactly one matching migration claim for the same workflow run;
- check out exactly the approved commit;
...
- active issue #657 must contain exactly one matching prior `NOVELIGHT_PRODUCTION_MIGRATION_DEPLOY_DISPATCHED` record for that old SHA and `supabase-production.yml` target;
...
A waiting run whose only supporting evidence exists on legacy issue #165 or #460 is not silently trusted after rotation; cleanup fails closed and requires explicit investigation. No unrelated Actions run may be cancelled through this cleanup. If no stale waiting run exists, cleanup is a no-op. The cleanup itself is not Production database mutation and cannot approve or execute a migration.
...
- trigger: a new owner-authored dispatch approval record on active Production Approval Ledger issue #657
- approved ref: exact current `main` SHA
...
Issue #657 is the active shared Production Approval Ledger and must never be closed by the Auth Smoke dispatch bridge. Issues #165 and #460 are retained only as legacy audit history; the retired fixed-baseline route remains pinned to #165.
