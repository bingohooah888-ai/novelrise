...
`STAGING_DATABASE_URL` はGitHub Actions Secretとしてのみ保持し、Vercel Preview、browser code、Issue、artifact、ログへ複製しない。
...
3. `STAGING_SUPABASE_URL` と `STAGING_DATABASE_URL` が同じ専用Staging projectを指すことを検証し、Production projectを明示的に拒否する。
...
6. Stagingのactual pending setが入力したexact setと完全一致することを確認する。
