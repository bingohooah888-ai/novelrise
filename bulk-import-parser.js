...
function parseWithHeadings(text) {
  ...
  for (const line of normalizeImportText(text).split('\n')) {
...
function parseWithDelimiter(text, delimiter) {
  ...
  return chunks.map((chunk, index) => {
    ...
    return {
      ...
      content: (first.length <= 150 ? chunkLines.slice(1) : chunkLines)
        .join('\n')
        .replace(/^\n+|\n+$/g, ''),
      included: true
    };
...
export function parseBulkEpisodes(text, options = {}) {
  ...
  return delimiter
    ? parseWithDelimiter(text, delimiter)
    : parseWithHeadings(text);
}
...
export function validateBulkEpisodes(items, options = {}) {
  ...
  if (!active.length) {
    errors.push('移行するエピソードを1話以上選択してください。');
  }
  if (active.length > maxEpisodes) {
    errors.push('一度に移行できるのは' + maxEpisodes + '話までです。');
  }
  ...
  active.forEach((item, index) => {
    ...
    if (title.length > 150) {
      errors.push(label + ': タイトルは150文字以内にしてください。');
    }
    if (!body.trim()) {
      errors.push(label + ': 本文が空です。');
    }
    if (body.length > maxChars) {
      errors.push(
        label +
          ': 本文は' +
          maxChars.toLocaleString('ja-JP') +
          '文字以内にしてください。'
      );
    }
  });
...
export function duplicateWarnings(items, existing = []) {
  ...
  active.forEach((item, index) => {
    const title = String(item.title ?? '');
    const body = String(item.content ?? '');
    const exact = title + '\u0000' + body;

    if (seen.has(exact)) {
      warnings.add(
        index +
          1 +
          '話目は、今回の移行内の別エピソードとタイトル・本文が一致しています。'
      );
    } else {
      seen.set(exact, index);
    }

    if ((existing ?? []).some((row) => String(row.content ?? '') === body)) {
      warnings.add(
        index +
          1 +
          '話目は、既存エピソードと本文が一致する可能性があります。'
      );
    }
    ...
    if (
      (existing ?? []).some(
        (row) =>
          ...
          String(row.content ?? '') === body
      )
    ) {
      warnings.add(
        index +
          1 +
          '話目は、既存エピソードとタイトル・本文が一致しています。'
      );
    }
  });
